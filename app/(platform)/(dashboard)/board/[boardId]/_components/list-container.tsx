...
export const ListContainer = ({boardId, data}: ListContainerProps) => {
	...
	const onDDragEnd = (result: any): void => {
		...
		if(type === 'card') {
			...
			/// Check if cards exists on the source
			if(!sourceList.cards) {
				sourceList.cards = [];
			}
			/// Check if cards exists on the destination
			if(!destList.cards){
				destList.cards = [];
			}
			...
			if(source.droppableId === destination.droppableId) {
				...
				executeUpdateCardOrder({ boardId:boardId, items: reorderedCards}).then();

				/// user moves the cards to another list
			} else {
				/// remove card from the source list
				const [movedCard] = sourceList.cards.splice(source.index, 1);
				/// Assign new list id to the new card
				...
				executeUpdateCardOrder({
					boardId: boardId, items: destList.cards
				}).then();
			}
		}
	};
